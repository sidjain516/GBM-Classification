...
def ithDiagonal(i, m1P, m2P, v1, v2, maxDel):
    ...
    while (slotIter[0] >= 0 and slotIter[1] <= m2P-1) or slotIter == start:
        # Artificial data restriction:
        if abs(slotIter[0]*v1 - slotIter[1]*v2) <= maxDel or \
            abs((slotIter[0]+1)*v1-1 - slotIter[1]*v2) <= maxDel or \
            abs(slotIter[0]*v1 - ((slotIter[1]+1)*v2-1)) <= maxDel or \
            maxDel == -1:
