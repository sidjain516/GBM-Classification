...
class DPdataClass:
    def __init__(self, seq1, seq2, v1, v2, gapScore, m1P, m2P, maxDel):
        ...
        # Fill out the top rows and leftmost columns of scoreMatrix and btMatrix
    ...
    def printAlignment(self):
        ...
        t = (len(self.seq1), len(self.seq2))
        while self.getBT(t[0], t[1]) != -1:
            if self.getBT(t[0], t[1]) == 'l':
                seq1Aligned = [delString] + seq1Aligned
                seq2Aligned = [(self.seq2[t[1] - 1]).__str__()] + seq2Aligned
                t = (t[0], t[1] - 1)
            if self.getBT(t[0], t[1]) == 'u':
                seq1Aligned = [(self.seq1[t[0] - 1]).__str__()] + seq1Aligned
                seq2Aligned = [delString] + seq2Aligned
                t = (t[0] - 1, t[1])
            if self.getBT(t[0], t[1]) == 'd':
                seq1Aligned = [(self.seq1[t[0] - 1]).__str__()] + seq1Aligned
                seq2Aligned = [(self.seq2[t[1] - 1]).__str__()] + seq2Aligned
                t = (t[0] - 1, t[1] - 1)
        ...
        printstr = []
        for j in range(len(seq1Aligned)):
            if seq1Aligned[j][1:3] == 'QQ' or seq2Aligned[j][1:3] == 'QQ':
                break
            printstr.append("{0:30}".format(seq1Aligned[j]))
            printstr.append("{0:30}".format(seq2Aligned[j]))
            printstr.append("\n")
    ...
    def infoToSlot(self, slot):
        ...
        return {'dScore': self.getScore(ulCorner1 - 1, ulCorner2 - 1),
                'uScores': [self.getScore(ulCorner1 - 1, ulCorner2 + j) for j in range(self.v2)],
                'lScores': [self.getScore(ulCorner1 + j, ulCorner2 - 1) for j in range(self.v1)]}
    ...
    def retrieveResults(self, res):
        ...
        for i in range(self.v1):
            for j in range(self.v2):
                self.putScore(slot[0] * self.v1 + 1 + i, slot[1] * self.v2 + 1 + j, scoreSubmatrix[i][j])
                self.putBT(   slot[0] * self.v1 + 1 + i, slot[1] * self.v2 + 1 + j, btSubmatrix[i][j])
    ...
    def putScore(self, i, j, val):
        # If someone is trying to out data outside the data restriction, just return.
    ...
    def getBT(self, i, j):
        ...
        if (i, j) not in self.btMatrix:
            print "Bad key-value in btMatrix:", i, j, ". Consider enlarging maxDel."
            if i>j: return 'u'
            if i<j: return 'l'
            if i == j: return 'd'
